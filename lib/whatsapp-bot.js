...
export function normalizeGroupJid(value) {
  ...
  if (!raw) return '';
  const groupId = raw.replace(/@g\.us$/, '').replace(/@.*$/, '').replace(/[^0-9-]/g, '');
...
export async function resolveParticipantSenderJid(socket, participantJid) {
  ...
  try {
    return await socket?.signalRepository?.lidMapping?.getPNForLID(senderJid) || senderJid;
  } catch (error) {
...
export function isAuthorizedSender(senderJid, authorizedSenders) {
  ...
  return normalized && normalizeAuthorizedSenders(authorizedSenders).includes(normalized);
}

export function resolveWhatsappSenderUser(senderJid, users = []) {
  const normalized = normalizeSenderJid(senderJid);
  const matches = users.filter((item) => isAuthorizedSender(normalized, item.settings?.whatsappAuthorizedSenders));
  if (!matches.length) return { type: 'none', senderJid: normalized };
  if (matches.length > 1) return { type: 'conflict', senderJid: normalized, matches };
  return { type: 'matched', senderJid: normalized, ...matches[0] };
}

export function resolveWhatsappPhoneUser(senderJid, users = []) {
  const phoneNumber = isWhatsappLidJid(senderJid) ? '' : normalizeWhatsappPhone(senderJid);
  const matches = phoneNumber
    ? users.filter((item) => normalizeWhatsappPhone(item.user?.whatsappPhoneNumber) === phoneNumber)
    : [];
  const sender = isWhatsappLidJid(senderJid) ? String(senderJid || '').trim() : normalizeSenderJid(senderJid);
  if (!matches.length) return { type: 'none', senderJid: sender, phoneNumber };
  if (matches.length > 1) return { type: 'conflict', senderJid: sender, phoneNumber, matches };
...
function reminderLeadMinutes(settings) {
  const lead = Number(settings.whatsappReminderLeadMinutes ?? DEFAULT_WHATSAPP_SETTINGS.whatsappReminderLeadMinutes);
  if (!Number.isFinite(lead)) return DEFAULT_WHATSAPP_SETTINGS.whatsappReminderLeadMinutes;
...
export function buildDailyReminderState(settings = {}, previousState = {}, now = new Date(), random = Math.random) {
  const dateKey = localDateKey(now);
  if (previousState?.dateKey === dateKey
  ...
  for (const action of ACTIONS) {
    ...
    dailyReminderTimes[action] = timeFromMinutes(minutesFromTime(target) - lead);
...
export function getDueReminders(settings = {}, previousState = {}, now = new Date(), random = Math.random) {
  const state = structuredClone(buildDailyReminderState(settings, previousState, now, random));
  const reminders = [];
  ...
  for (const action of ACTIONS) {
    if (!state.remindersSent[action] && isReminderDue(current, state.dailyReminderTimes[action])) {
...
export function parseWhatsappCommand(text) {
  ...
  if (['status', 'cek', 'session'].includes(normalized)) return { type: 'status' };

  const clockIn = normalized.match(/^(?:clock in|check in|ci)(?:\s+(random|photo\s+\d+))?$/);
  if (clockIn) {
  ...
  if (/^(?:clock out|check out|co)$/.test(normalized)) return { type: 'clock-out' };
  return { type: 'unknown' };
}
...
async function resolveClockInPhoto(command, settings, deps) {
  ...
  if (command.useRandomPhoto || settings.randomPhotoEnabled) {
    ...
    if (!photos.length) return { error: 'Belum ada foto tersimpan untuk clock-in random.' };
    const photo = photos[Math.floor(Math.random() * photos.length)];
    return { photoId: String(photo.id) };
  }

  if (settings.selectedPhotoId) {
    const photo = deps.getPhoto ? await deps.getPhoto(settings.selectedPhotoId) : null;
    if (!photo) return { error: `Foto terpilih #${settings.selectedPhotoId} tidak ditemukan.` };
    return { photoId: String(photo.id) };
  }

  return { error: 'Belum ada foto untuk clock-in. Pakai `clock in random` atau `clock in photo <id>`.' };
}

function formatStatus(session, dashboard) {
  if (!session.loggedIn) return 'HRIS status:\nSession: belum login. Login dulu dari aplikasi lokal.';
  return [
    'HRIS status:',
    'Session: logged in',
...
export async function handleWhatsappCommand({ senderJid, text, settings, deps, authorized = false }) {
  if (!authorized && !isAuthorizedSender(senderJid, settings.whatsappAuthorizedSenders)) return null;
  ...
  if (command.type === 'clock-in') {
    ...
    const payload = await deps.storeClockIn({ photoId: photo.photoId });
    return `Clock-in sukses.\n${JSON.stringify(payload, null, 2)}`;
  }

  if (command.type === 'clock-out') {
    const payload = await deps.storeClockOut({});
    return `Clock-out sukses.\n${JSON.stringify(payload, null, 2)}`;
  }

  return 'Command tidak dikenal. Kirim `help`.';
}

export async function sendReminders({ socket, settings, deps, now = new Date(), random = Math.random }) {
  ...
  const { state, reminders } = getDueReminders(settings, previousState, now, random);
  const changed = JSON.stringify(state) !== JSON.stringify(previousState);

  if (reminders.length) {
    for (const reminder of reminders) {
      await socket.sendMessage(groupJid, { text: formatReminderMessage(reminder) });
    }
  }
...
export async function startWhatsappBot(deps) {
  ...
  async function connect() {
    ...
    socket.ev.on('connection.update', (update) => {
      ...
      if (update.connection === 'close' && !stopped && shouldReconnect(update.lastDisconnect)) {
    ...
    socket.ev.on('messages.upsert', async ({ messages }) => {
      for (const message of messages || []) {
        ...
        if (!remoteJid || remoteJid === 'status@broadcast' || message.key?.fromMe) continue;
        ...
        try {
          ...
          const resolvedSenderJid = await resolveParticipantSenderJid(socket, senderJid);
          const command = parseWhatsappCommand(text);
          ...
          if (target.type === 'none') {
            if (!['empty', 'unknown'].includes(command.type)) {
              await socket.sendMessage(remoteJid, { text: 'Nomor WhatsApp kamu belum terdaftar di aplikasi.' });
            }
            continue;
          }
          if (target.type === 'conflict') {
            await socket.sendMessage(remoteJid, { text: 'Nomor WhatsApp ini terhubung ke lebih dari satu user aplikasi. Hapus duplikat nomor dulu.' });
            continue;
          }
          ...
          await socket.sendMessage(remoteJid, { text: `HRIS bot error: ${error.message}` });
        }
      }
    });
  }
      ...
      if (deps.listReminderTargets) {
        ...
        for (const target of targets) {
          if (target.settings.whatsappBotEnabled) {
            await sendReminders({ socket, settings: target.settings, deps: target.deps });
          }
        }
        return;
      }
