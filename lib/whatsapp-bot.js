...
export function normalizeWhatsappPhone(value) {
  return String(value || '').trim().toLowerCase().replace(/@s\.whatsapp\.net$/, '').replace(/[^0-9]/g, '');
}
      ...
      for (const message of messages || []) {
        ...
        try {
          ...
          const target = deps.resolveSender
            ? await deps.resolveSender(senderJid, groupJid)
